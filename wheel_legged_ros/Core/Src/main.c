...
void HAL_UART_IDLECallback(UART_HandleTypeDef *huart)
{
    if(huart->Instance == USART1)
    {
        ...
        uint16_t recv_len = UART_RX_BUFFER_SIZE - __HAL_DMA_GET_COUNTER(huart->hdmarx);

        for(uint16_t i = 0; i < recv_len; i++)
        {
            ParseByte(UART_RxBuffer[i]);
        }

        //  ������ֹͣ���������� DMA���������һ�ξ���
        HAL_UART_Receive_DMA(huart, UART_RxBuffer, UART_RX_BUFFER_SIZE);
    }
}
...
/* Configure the peripherals common clocks */
  ...
  FDCAN1_Config();//can
	FDCAN2_Config();
	HAL_UART_Receive_DMA(&huart1, UART_RxBuffer, UART_RX_BUFFER_SIZE);
  /* USER CODE END 2 */
