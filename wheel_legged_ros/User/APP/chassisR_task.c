...
void ChassisR_task(void)
{
  ...
  Pensation_init(&Roll_Pid,&Tp_Pid,&Turn_Pid);//补偿pid初始化
	...
	{	
		...
		float dt = (float)CHASSR_TIME * 3.0f / 1000.0f;  // 任务周期 = 0.003s

		if (chassis_move.ctrl_mode == 0)   // ROS控制模式
		{
				chassis_move.v_set = ros_vx;   
				chassis_move.x_set=chassis_move.x_set+chassis_move.v_set*dt;
				chassis_move.turn_set += ros_wz * dt;   // turn_set 单位是角度(rad)
