...
{   
 ...
 if(chassis->recover_flag==0
  ...
  if(data->last_key!=1&&data->key==1&&chassis->prejump_flag==0&&chassis->start_flag==1) 
	...
	if(data->last_key!=PSB_GREEN && data->key==PSB_GREEN) //PSB绿键被按下
	{
	  chassis->ctrl_mode = ! chassis->ctrl_mode;//控制模式切换
	}
	
	// 添加PSB_L1按键触发跳跃（PSB_L1对应MASK中的索引为10，对应key值为11）
	if(data->last_key!=PSB_L1&&data->key==PSB_L1&&chassis->jump_flag==0&&chassis->jump_flag2==0)
	{
			chassis->jump_flag=1;
			chassis->jump_flag2=1;
	}
